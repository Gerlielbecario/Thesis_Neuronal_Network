...
def plot_losses(Dir_plot, max_epochs, loss_train, loss_val):

    fig = plt.figure(figsize=(8,4))

    plt.plot(range(max_epochs), loss_train, label="Train", color="blue")
    plt.plot(range(max_epochs), loss_val, label="Val", color="red")
    plt.xlim(0,max_epochs)
    plt.xticks(np.arange(0,max_epochs+10,10))
    plt.legend()
    plt.grid()
                
    plt.savefig(Dir_plot+"Losses.png",dpi=100,bbox_inches="tight")
    plt.close()
