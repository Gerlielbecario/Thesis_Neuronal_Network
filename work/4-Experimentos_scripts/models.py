...
class decoder_block(torch.nn.Module):
    def __init__(self, in_c, out_c , kernel_size = 3 , padding = 1 , decotype = 'conv' , act_type = 'ReLU' , bias = False ) :
        super().__init__()
        if decotype == 'bilinear' :
           self.up = torch.nn.Upsample( scale_factor=kernel_size , mode='bilinear' )
        elif decotype == 'bicubic' :
           self.up = torch.nn.Upsample( scale_factor=kernel_size , mode='bicubic' )
        elif decotype == 'nearest' :
           self.up = torch.nn.Upsample( scale_factor=kernel_size , mode='nearest' )        
...
class decoder_block_skip(torch.nn.Module):
    def __init__(self, in_c, out_c , kernel_size = 3 , padding = 1 , decotype = 'bilinear' , act_type = 'ReLU' , bias = False ):
        ...
        if self.decotype == 'bilinear' :
           self.upsample = torch.nn.Upsample
        elif self.decotype == 'bicubic' :
           self.upsample = torch.nn.Upsample
        elif self.decotype == 'nearest' :
           self.upsample = torch.nn.Upsample
...
class EncoderDecoder(torch.nn.Module):
    ...
    def __init__(self, model_conf ):
        ...
        if 'Channels' in model_conf.keys() and len(model_conf['Channels'] ) == 7 :
            self.channels = model_conf['Channels']
        else : 
            print('Warning: Using default number of input channels')
            self.channels = [1,16,32,64,128,64,1]
        
        if 'KernelSize' in model_conf.keys() and len(model_conf['KernelSize'] ) == 6 :
            self.kernel_size = model_conf['KernelSize']
        else :
            print('Warning: Using default kernel sizes')
            self.kernel_size = np.array([3,3,2,2,1])
        ...
        self.model_layers = torch.nn.ModuleList()
        self.model_layers.append( encoder_block( self.channels[0] , self.channels[1] , pool = self.pool , kernel_size = self.kernel_size[0] , padding = self.padding[0] ) , act_type = self.act_type , bias = self.bias )
        self.model_layers.append( encoder_block( self.channels[1] , self.channels[2] , pool = self.pool , kernel_size = self.kernel_size[1] , padding = self.padding[1] ) , act_type = self.act_type , bias = self.bias )
        
        self.model_layers.append( decoder_block( self.channels[2] , self.channels[3] , kernel_size = self.kernel_size[2] , padding = self.padding[2] ) , act_type = self.act_type , bias = self.bias )
        self.model_layers.append( decoder_block( self.channels[3] , self.channels[4] , kernel_size = self.kernel_size[3] , padding = self.padding[3] ) , act_type = self.act_type , bias = self.bias )
        self.model_layers.append( conv_block( self.channels[4] , self.channels[5] , kernel_size = self.kernel_size[4] , padding = self.padding[4] ) , act_tpye = self.act_type , bias = self.bias )
        self.model_layers.append( torch.nn.Conv2d( self.channels[5] , self.channels[6] , kernel_size=self.kernel_size[5] ,stride = 1, padding= self.padding[5], bias= self.bias , padding_mode='reflect') )
        
        exec('self.out_act = torch.nn.' + out_act_type + '()' )
        self.model_length = len( self.model_layers ) 
        
    def forward(self, x):
        batch = x.shape[0]
        x = x.view(batch,1,self.nx,self.ny) # x - Shape 4D: (batch size, filtros, nx, ny)
        for ii in range( self.model_length ) :
            x = self.model_layers[ii]( x )
        x = self.out_act( x )
...
class unet(torch.nn.Module):
    def __init__(self, model_conf ):
        ...
        self.nx = model_conf['Nx']
        self.ny = model_conf['Ny']
        Keys = ['InActivation' , 'OutActivation' , 'DecotType' , 'Pool' , 'KernelSize' ]
        if 'InActivation' in model_conf.keys() :
           self.act_type = model_conf['InActivation']
        else :
           self.decotype = 'ReLU'
